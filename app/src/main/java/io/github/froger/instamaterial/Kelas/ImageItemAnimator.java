...
/**
 * Created by Glory on 20/07/2018.
 */

public class ImageItemAnimator extends DefaultItemAnimator {
        ...
        if (changeFlags == FLAG_CHANGED) {
            for (Object payload : payloads) {
                if (payload instanceof String) {
                    return new ImageItemAnimator.FeedItemHolderInfo((String) payload);
                }
            }
        }
    ...
    public boolean animateChange(@NonNull RecyclerView.ViewHolder oldHolder,
        ...
        if (preInfo instanceof FeedItemHolderInfo) {
            ...
            ImageAdapter.ViewHolder holder = (ImageAdapter.ViewHolder) newHolder;

            animateHeartButton(holder);
            // updateLikesCounter(holder, holder.getFeedItem().likesCount);
            if (ImageAdapter.ACTION_LIKE_IMAGE_CLICKED.equals(feedItemHolderInfo.updateAction)) {
    ...
    private void animatePhotoLike(final ImageAdapter.ViewHolder holder) {
    ...
    private void animateHeartButton(final ImageAdapter.ViewHolder holder) {
        AnimatorSet animatorSet = new AnimatorSet();

        ObjectAnimator rotationAnim = ObjectAnimator.ofFloat(holder.mLikeButton, "rotation", 0f, 360f);
        rotationAnim.setDuration(300);
        rotationAnim.setInterpolator(ACCELERATE_INTERPOLATOR);

        ObjectAnimator bounceAnimX = ObjectAnimator.ofFloat(holder.mLikeButton, "scaleX", 0.2f, 1f);
        bounceAnimX.setDuration(300);
        bounceAnimX.setInterpolator(OVERSHOOT_INTERPOLATOR);

        ObjectAnimator bounceAnimY = ObjectAnimator.ofFloat(holder.mLikeButton, "scaleY", 0.2f, 1f);
    ...
    private void dispatchChangeFinishedIfAllAnimationsEnded(ImageAdapter.ViewHolder holder) {
    ...
    private void resetLikeAnimationState(ImageAdapter.ViewHolder holder) {
    ...
    private void runEnterAnimation(final ImageAdapter.ViewHolder holder) {
    ...
    public boolean animateAdd(RecyclerView.ViewHolder viewHolder) {
        if (viewHolder.getItemViewType() == 0) {
            if (viewHolder.getLayoutPosition() > lastAddAnimatedItem) {
                lastAddAnimatedItem++;
                runEnterAnimation((ImageAdapter.ViewHolder) viewHolder);
                return false;
            }
        }
